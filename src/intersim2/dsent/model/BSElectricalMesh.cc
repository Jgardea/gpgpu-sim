...
namespace DSENT
{
    ...
    void BSElectricalMesh::initProperties()
    {
		...
		addPropertyName("TSV->Length");
		addPropertyName("TSV->Diameter");
		addPropertyName("TSV->Pitch");
		addPropertyName("TSV->BumpDiameter");
        return;
    }
    ...
    void BSElectricalMesh::constructModel()
    {
		...
		getGenProperties()->set("NumberTSV", 1);
    ...
    void BSElectricalMesh::updateModel()
    {
		...
		double ver_link_bump_diameter = getProperty("TSV->BumpDiameter");
		...
		ver_link->setProperty("Length", ver_link_length);
		ver_link->setProperty("Diameter", ver_link_diameter);
        ...
        if ( isAsymmetric() )
        {
			...
			asym_ver_link->setProperty("Length", ver_link_length);
			asym_ver_link->setProperty("Diameter", ver_link_diameter);
