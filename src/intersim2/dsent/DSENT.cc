...
namespace DSENT
{
    ...
    bool DSENT::is_asymmetric = false;
    vector<const Result*> DSENT::ms_results;
    vector< pair<String, vector<pair<String, double> > > >  DSENT::ms_energy_results;

    vector< pair<String, vector<pair<String, double> > > > DSENT::run(vector<string> argv_)
    {
    ...
    void DSENT::initialize(vector<string> argv_)
    {
        ...
        // Parse the options
        option_parser->parseArguments(argv_);
...
// ********************* BookSim ************************* // jgardea
    ...
    void DSENT::getResults( )
    {
        vector< pair<String, vector<pair<String, double> > > > * temp_results;
        pair<String, vector<pair<String, double> > > component;
        int component_index;
        ...
        for ( unsigned i = 0; i < ms_results.size(); i++  )
        {
            ...
            for ( unsigned j = 0; j < temp_results->size(); j++)
            {
                ...
                component_index = findComponent(component.first);
                
                if ( component_index  >= 0 )
                {
                    vector<pair<String, double> >* events = &ms_energy_results[component_index].second;
                    events->insert(events->begin(), component.second.begin(), component.second.end() );
                }
                else ms_energy_results.push_back(temp_results->at(j));   
            }
        }
    }

    double DSENT::findResult( string component, string event )
    {
        int index = findComponent(String(component));
        if ( index < 0 )
        {
            std::cerr << "Could not find component \'" << component << "\'" << endl;
            return -1.0;
        }

        vector<pair<String, double> > comp_events = ms_energy_results[index].second; 
        
        for ( unsigned i = 0; i < comp_events.size(); i++ )
        {
            if ( comp_events[i].first == event )
            {
                return comp_events[i].second;
            }
            
        }
        std::cerr << "Did not find Event \'" << event << endl;
        return -1.0;
    }
    ...
    void DSENT::printResults( )
    {
        ...
        {
            pair<String, vector<pair<String, double> > > component = ms_energy_results[i];
            std::cout <<  "Component: " << component.first << endl;
            vector<pair<String, double> > event = component.second;

            for ( unsigned j = 0; j < event.size(); j++ )
            {
                std::cout << "  Event: " << event[j].first << " Results: " << event[j].second << endl;
            }
        }
    }

    int  DSENT::findComponent( String comp )
    {
        int found = -1;
        for ( unsigned i = 0; i < ms_energy_results.size(); i++ )
        {
            if ( comp == ms_energy_results[i].first )
                found = i;
        }
        return found;
    }
