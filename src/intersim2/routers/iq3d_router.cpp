...
IQ3DRouter::IQ3DRouter( Configuration const & config, Module *parent, 
		    string const & name, int id, int inputs, int outputs, RoundRobinArbiter* up_arbiter, RoundRobinArbiter* down_arbiter ) //jgardea
: Router( config, parent, name, id, inputs, outputs ), _active(false)
{
  ...
  _verc_arb_delay = config.GetInt( "verc_arb_delay"); //jgardea

  if ( gS == 1 ) _zvalue = 0;
  else _zvalue = _id/(gK*gN);  //jgardea 
  ...
  _upc_arbiter = up_arbiter;
  _downc_arbiter = down_arbiter;
...
//void IQ3DRouter::AddOutputChannel(FlitChannel * channel, CreditChannel * backchannel, bool isVertical)
void IQ3DRouter::AddOutputChannel(FlitChannel * channel, CreditChannel * backchannel )
{
  ...
  Router::AddOutputChannel(channel, backchannel, true);
}

void IQ3DRouter::ReadInputs( )
{
...
void IQ3DRouter::Evaluate_AfterSA( )
{
  _partial_internal_cycles += _internal_speedup; // jgardea internal steps after vertical allocation for IQ3D Router 
  while( _partial_internal_cycles >= 1.0 ) {
    _InternalStep_AfterSA( );
...
void IQ3DRouter::_InternalStep( )
{
  ...
  _active_AfterSA = activity;
}

// -----------------------------------------------------------------------------
// For Vertical Channel Arbitration the internal steps of each router have to be splitted in two 
// pre switch allocation and post switch allocation steps. 
// This is because Vertical Allocation affects all the routers connected to the vertical bus, so 
// the input to the vertical channel arbiter is the output of the switch allocation stage for each router.

//jgardea 
void IQ3DRouter::_InternalStep_AfterSA( )  
{
  if ( !_active ) return;

  //cout << endl <<  GetSimTime() <<  " | R[" << _id <<  "] Entering internal Steps 2 " << endl;

  int activity = _active_AfterSA;

  _VerChannelArbitration( );  // jgardea 
...
bool IQ3DRouter::_ReceiveFlits( )
{
  ...
  for(int input = 0; input < _inputs; ++input) { 
    ...
    if(f) {
      // This snippet of code make sures that only the destination router processes 
      // the coming flit from vertical channels
      // jgardea
      ...
      if ( input_direction == _up || input_direction == _down )  
      {
        int fdest = f->dest;
        int fpid = f->pid; 
        int pid_index = _findPckt( fpid );

        int router_z = _id / (gK*gN);

        if( (router_z != (fdest/(gK*gN))) &&  (fdest != -1)) continue;
        if ( (fdest == -1) && ( pid_index == -1) ) continue;

        if ( f->head ) _currentPckt.push_back( fpid ); //jgardea
        if ( f->tail ) 
        {
          pid_index = _findPckt( fpid );
          assert( pid_index >= 0 );
          _currentPckt.erase( _currentPckt.begin() + pid_index );
        }
      }
      //jgardea
      ...
      if ( debug ){         //jgardea DEBUGGING                   
        cout << "\n" << GetSimTime() << " | " << "Router_" << _id << " | "
        << FullName() << " | "
        << "Receive f: " << f->id
        << " | pckt: " << f->pid
        << " | input: " << input
...
// Helper function to correclty handle flits
// coming from other layers ( vertical traffic )

int IQ3DRouter::_findPckt( int pid  ) //jgardea
{
  for ( unsigned pckt = 0; pckt < _currentPckt.size(); pckt++ )
  {
    if ( pid == _currentPckt[pckt] ) return pckt;
  }
  return -1;
}

bool IQ3DRouter::_ReceiveCredits( )
{
  ...
  for(int output = 0; output < _outputs; ++output) {  
    ...
    if(c) {
      ...
      //jgardea
      int output_direction = GetOutportDir(output);    
      if ( output_direction == _up )   
      {
        if ( c->prevRouter != _id  )
        {
          continue;
      ...
      else if ( output_direction == _down )
      {
        if( c->prevRouter != _id )
        {
          continue;
...
void IQ3DRouter::_InputQueuing( )
{
      ...
      ++iter) 
  {
    ...
    if ( debug ) 
    {
      ...
      if ( cur_buf->Empty(vc) ) cout << ", empty";
      else cout << ", front: " << cur_buf->FrontFlit(vc)->id;
      cout << ")." << endl;
    }
...
void IQ3DRouter::_RouteEvaluate( )
{
...
void IQ3DRouter::_VCAllocEvaluate( )
{
      ...
      for(int out_vc = vc_start; out_vc <= vc_end; ++out_vc) 
      {
        ...
        { //jgardea 
          ...
          { // jgardea
            ...
            cout << GetSimTime() << " | " << FullName() << " | ";
            cout  << "  VC " << out_vc ;
            cout   << " at output " << out_port ;
            
            if ( out_vc%gS == _zvalue )
            {
              cout   << " is in use by VC " << use_vc;
              cout   << " at input " << use_input;
              Flit * cf = _buf[use_input]->FrontFlit(use_vc);
              if(cf) {
                cout <<  " (front flit: " << cf->id << ")";
              } else {
                cout << " (empty)";
              }
            }
...
void IQ3DRouter::_VCAllocUpdate( )
{
  assert(_vc_allocator);
...
void IQ3DRouter::_SWHoldEvaluate( )
{
  ...
  while(!_sw_hold_vcs.empty()) {
    ...
    if(expanded_output >= 0 && ( _output_buffer_size==-1 || _output_buffer[expanded_output].size()<size_t(_output_buffer_size))) {
      ...
      if(!_routing_delay && f->head) {
        ...
        const Router * router = channel->GetSink(); //TODO fix later for vertical channels
        if(router) 
        {
          if(_noq) 
          {
          ...
          else 
          {
            ...
            int in_channel = channel->GetSinkPort(); // TODO fix later for vertical channels 
...
bool IQ3DRouter::_SWAllocAddReq(int input, int vc, int output)
{
  assert(input >= 0 && input < _inputs);
  assert(vc >= 0 && vc < _vcs);
  assert(output >= 0 && output < _outputs);
...
void IQ3DRouter::_SWAllocEvaluate( )
{
  bool watched = false;
      ...
      ++iter) 
  {
    ...
    {  
      ...
      bool const requested = _SWAllocAddReq(input, vc, dest_output);                          // Adding SWAllocation request
      watched |= requested && f->watch;
      continue;
    }
    assert(_speculative && (cur_buf->GetState(vc) == VC::vc_alloc));
    assert(f->head);                                                                          // Starts speculative part of SWAllocation
  ...
  }// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  for loop to add requests speculative implementation
 
  if ( debug )  // jgardea
  {
    cout << GetSimTime() << " | " << _sw_allocator->FullName() << " | ";
    _sw_allocator->PrintRequests( &cout );
  }
  ...
  _sw_allocator->Allocate();                                                                     // switch allocation 
  ...
  {
    cout << GetSimTime() << " | " << _sw_allocator->FullName() << " | ";
    _sw_allocator->PrintGrants( &cout );
  }
    ...
    if(expanded_output >= 0) 
    {
      ...
      if(granted_vc == vc) 
      {
        ...
        if ( outport_dir == _up || outport_dir == _down )
        {
          ...
          _AddVerticalRequest( outport_dir, expanded_input, vc, expanded_output, index );
          continue;
        }
        ...
        // ----- Here is were output is assigned ------- jgardea 
      } 
      else 
      {
        if ( debug )
  ...
  } // finishing allocation sets output if request was granted
  
  if(!_speculative && (_sw_alloc_delay <= 1)) {       // ------------------  None speculative implementaiton ENDS here
    return;
  }
...
void IQ3DRouter::_SWAllocUpdate( )
{
  while(!_sw_alloc_vcs.empty()) {

    pair<int, pair<pair<int, int>, int> >  & item = _sw_alloc_vcs.front(); //jgardea got rid of const
    ...
    if(expanded_output >= 0) { // if true, flit is ready for switch
      ...
      if(!_vc_allocator && (cur_buf->GetState(vc) == VC::vc_alloc)) // for piggyback
      {
        assert(f->head);
      ...
      f->hops++;                                  // increasing hope count
      f->vc = match_vc;                           // updating input vc for output vc

      if(!_routing_delay && f->head) 
      {
        const Router * router; 
        const VerticalChannel * ver_channel;
        const FlitChannel * channel = _output_channels[output]; 
        ...
        if ( outport_dir == _up || outport_dir == _down  ) 
        {
          ver_channel = (VerticalChannel*) channel;
          router = ver_channel->GetVerticalSink(f->dest);
        }else router = channel->GetSink();
        ...
        if(router) {
          if(_noq) {
            ...
            _rf(router, f, in_channel, &f->la_route_set, false); //TODO _rf
          }
        } else {
          f->la_route_set.Clear();
        }
      ...
      _out_queue_credits.find(input)->second->prevRouter = f->prevRouter; //jgardea need to keep track of flits for vertical credit transfer
      ...
      if(debug) 
      {
	      cout << GetSimTime() << " | " << FullName() << " | "
            ...
            assert((expanded_output == -1) || // for stalls that are accounted for in VC allocation path
             ...
             (expanded_output == STALL_VER_CROSSBAR_CONFLICT) || // jgardea
            ...
            } else if(expanded_output == STALL_CROSSBAR_CONFLICT || expanded_output == STALL_VER_CROSSBAR_CONFLICT) { // jgardea
              ++_crossbar_conflict_stalls[f->cl];
            }
#endif
        _sw_alloc_vcs.push_back(make_pair(-1, make_pair(item.second.first, -1))); // original
    }
...
//------------------------------------------------------------------------------
// Vertical Channel Arbitration
//------------------------------------------------------------------------------

void IQ3DRouter::_AddVerticalRequest( int direction, int input, int vc, int output, int index )
{
  // direction is to decide between arbitera
  // index is the index in the _sw_alloc_vcs vector to identify request
  
  RoundRobinArbiter *cur_arbiter = NULL; 
  vertical_req *ver_req = NULL;

  if ( direction == _up )
  {
    cur_arbiter = _upc_arbiter;
    ver_req = &_upc_vertical_req;
  }
  else if ( direction == _down )
  {
    cur_arbiter = _downc_arbiter;
    ver_req = &_downc_vertical_req;
  }
  else Error( "Direction not allowed in vertical channel" );

  ver_req->input = input;
  ver_req->vc = vc;
  ver_req->output = output;
  ver_req->index = index;

  cur_arbiter->AddRequest( _zvalue, 0, 0);

  if ( debug ) cout << "\t" << GetSimTime() << " | R[ " << _id << "] Adding " << ( (direction == _up) ? "Up" : "Down" ) <<  " Vertical Request input: " << input << " VC: " << vc << endl;
}

void  IQ3DRouter::_VerChannelArbitration( )
{
  int winner;
  int input;
  int vc;
  ...
  if ( _upc_arbiter->HasRequest( _zvalue ) )
  {
    index = _upc_vertical_req.index;
    iter = _sw_alloc_vcs.begin() + index;

    iter->first = GetSimTime() + _verc_arb_delay; 
    
    if ( _upc_arbiter->Winner() == -1 )
    {
      winner = _upc_arbiter->Arbitrate( 0 );
    }
    else
    {
      winner = _upc_arbiter->Winner( );
    }

    if ( winner == _zvalue )
    {
      input = _upc_vertical_req.input;
      vc = _upc_vertical_req.vc;
      output = _upc_vertical_req.output;

      _sw_rr_offset[input] = (vc + _input_speedup) % _vcs;
      iter->second.second = output;

      if ( debug ) cout << "\n\t" << GetSimTime() << "| Router_ " << _id << " Won Up Vertical Channel Allocation" << endl << endl;
    }
    else
    {
      if ( debug ) cout << "  Lost Up Arbitration for R[" << _id  << "],  Winner " << winner  << endl;
      iter->second.second = STALL_VER_CROSSBAR_CONFLICT;
    }
  }

  if ( _downc_arbiter->HasRequest( _zvalue ) )
  {
    index = _downc_vertical_req.index;
    iter = _sw_alloc_vcs.begin() + index;

    iter->first = GetSimTime() + _verc_arb_delay;
    
    if ( _downc_arbiter->Winner() == -1 )
    {
      winner = _downc_arbiter->Arbitrate( 0 );
    }
    else
    {
      winner = _downc_arbiter->Winner( );
    }
    
    if ( winner == _zvalue )
    {
      input = _downc_vertical_req.input;
      vc = _downc_vertical_req.vc;
      output = _downc_vertical_req.output;

      _sw_rr_offset[input] = (vc + _input_speedup) % _vcs;
      iter->second.second = output;

      if ( debug ) cout << "\n\t" << GetSimTime() << "| Router_ " << _id << " Won Down  Vertical Channel Allocation" << endl << endl;
    }
    else 
    {
      if ( debug ) cout << "  Lost Down Arbitration for R[" << _id  << "],  Winner " << winner  << endl;
      iter->second.second = STALL_VER_CROSSBAR_CONFLICT;
    }
  }
}
...
void IQ3DRouter::_SwitchEvaluate( )
{
...
void IQ3DRouter::_OutputQueuing( )
{
...
void IQ3DRouter::_SendFlits( )
{
  for ( int output = 0; output < _outputs; ++output ) {
    if ( !_output_buffer[output].empty( ) ) {
      ...
      _output_channels[output]->UpdateUtilization(); // jgardea
    }
  }
}

void IQ3DRouter::_SendCredits( )
{
  for ( int input = 0; input < _inputs; ++input ) {
    if ( !_credit_buffer[input].empty( ) ) {
      ...
      if ( _input_credits[input]->IsBusy() ) //jgardea
      {
        continue;
      }
...
void IQ3DRouter::ClearVerticalArbiters( )
{
    if ( _upc_arbiter->_num_reqs != 0 )
    {
      if (debug) cout << GetSimTime() << " | Clearing Up Arbiter " << _upc_arbiter->Name() << endl;
      _upc_arbiter->Clear();
    }
  
    if ( _downc_arbiter->_num_reqs != 0 )
    {
      if (debug) cout << GetSimTime() << " | Clearing Down Arbiter " << _downc_arbiter->Name() << endl;
      _downc_arbiter->Clear();
    }
}
...
void IQ3DRouter::Display( ostream & os ) const
{
  for ( int input = 0; input < _inputs; ++input ) {
...
int IQ3DRouter::GetUsedCredit(int o) const
{
...
int IQ3DRouter::GetBufferOccupancy(int i) const {
  assert(i >= 0 && i < _inputs);
  return _buf[i]->GetOccupancy();
}

#ifdef TRACK_BUFFERS
int IQ3DRouter::GetUsedCreditForClass(int output, int cl) const
{
  assert((output >= 0) && (output < _outputs));
  BufferState const * const dest_buf = _next_buf[output];
...
int IQ3DRouter::GetBufferOccupancyForClass(int input, int cl) const
{
  assert((input >= 0) && (input < _inputs));
...
vector<int> IQ3DRouter::UsedCredits() const
{
...
vector<int> IQ3DRouter::FreeCredits() const
{
  vector<int> result(_outputs*_vcs);
  for(int o = 0; o < _outputs; ++o) {
    for(int v = 0; v < _vcs; ++v) {
...
vector<int> IQ3DRouter::MaxCredits() const
{
  vector<int> result(_outputs*_vcs);
  for(int o = 0; o < _outputs; ++o) {
    for(int v = 0; v < _vcs; ++v) {
...
void IQ3DRouter::_UpdateNOQ(int input, int vc, Flit const * f) {
  assert(!_routing_delay);
  assert(f);
  assert(f->vc == vc);
  assert(f->head);
